...
class DaiHuiXingCheLiangVC: DDNormalVC {
    ...
    func prepareRequest() {
        DDQueryManager.share.daiHuiXing(type: ApiModel<CheYuanDataModel>.self, page: "1", findMsg: nil) { (result ) in
            mylog(result.msg)
...
extension DaiHuiXingCheLiangVC : UICollectionViewDelegate ,UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let vc = DaiHuiXingCheLiangStep2VC()
        vc.baseInfoModel = self.apiModel.data?.list?[indexPath.item] ?? DaiHuiXingCheLiangVC.CheYuanModel()
...
extension DaiHuiXingCheLiangVC{
